...
export class MCPExecutor {
  static async executeMCPTool({
    ...
    try {
      const response = await mcpManager.callTool(serverId, toolName, toolArguments);
      ...
      if (finalPrompt && instantModel) {
        const enhancedPrompt = `${finalPrompt}\n\nBased on the following MCP tool result:\n\n${content}`;
        
        await aiCompletionRunner({
          sourceUid,
          prompt: enhancedPrompt,
          additionalPrompt: "",
          command: `MCP Tool: ${toolName}`,
          instantModel,
          includeUids: false,
          target,
          style,
      ...
      const errorMessage = `Error executing MCP tool "${toolName}": ${error.message}`;
      await aiCompletionRunner({
        sourceUid,
        prompt: errorMessage,
        additionalPrompt: "",
        command: `MCP Tool Error: ${toolName}`,
