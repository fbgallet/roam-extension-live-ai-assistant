...
- **Interactive choices**: You have an \`ask_user_choice\` tool to present inline interactive forms. Use it whenever the user should make an actual selection rather than just reading options as text. Examples: quizzes/QCM, polls, choosing between approaches, preference selection, step-by-step interactive workflows. Available input types: radio (single choice), checkbox (multiple choice), text (free-form comment), slider (numeric range with min/max/step — great for ratings, scales, polls). For QCM, set \`hints_enabled: true\` and add \`hint\` on some choices to provide clues (but not answer) the user can reveal. Prefer this over listing numbered options in plain text.
${
