...
export const applyLinearContentTruncation = (
  ...
  const availableContentChars = Math.max(0, targetCharLimit - (data.length * reservedCharsPerItem));
  const maxContentPerItem = Math.max(50, Math.floor(availableContentChars / data.length));
  ...
  let totalChars = 0;
  const truncatedData = data.map((item, index) => {
    const baseItem = {
      uid: item.uid,
    ...
    const baseItemStr = `UID: ${baseItem.uid || baseItem.pageUid}, Title: ${baseItem.pageTitle || ''}, Count: ${baseItem.count || ''}`;
    ...
    if (item.content) {
      if (item.content.length <= maxContentPerItem) {
        ...
        truncatedContent = item.content.substring(0, maxContentPerItem) + '...';
      }
    }
    ...
    if (totalChars > targetCharLimit * 0.95) {
      console.log(
        `🎯 [LinearTruncation] Reached 95% of limit at item ${index + 1}, stopping here`
      );
  ...
  console.log(
    `🎯 [LinearTruncation] Reduced from ${data.length} to ${truncatedData.length} items, estimated ${totalChars} chars`
  );
...
export const applyIntermediateContentTruncation = (
  ...
  const availableContentChars = Math.max(0, targetCharLimit - (data.length * reservedCharsPerItem));
  const maxContentPerItem = Math.max(100, Math.floor(availableContentChars / data.length)); // Minimum 100 chars
  ...
  const truncatedData = data.map((item, index) => {
    ...
    const baseItemStr = `UID: ${baseItem.uid || baseItem.pageUid}, Title: ${baseItem.pageTitle || ''}, Type: ${baseItem.type || 'block'}, Count: ${baseItem.count || ''}`;
    ...
    if (item.isPage || item.type === 'page') {
      ...
      if (item.content.length <= maxContentPerItem) {
        ...
        const smartEnd = lastSpace > maxContentPerItem * 0.8 ? lastSpace : maxContentPerItem;
      ...
      const serializedHierarchy = typeof hierarchyContent === 'string' 
        ? hierarchyContent 
      ...
      if (serializedHierarchy.length <= maxContentPerItem) {
        ...
        const smartEnd = Math.max(truncated.lastIndexOf('}'), truncated.lastIndexOf(']'));
        const finalEnd = smartEnd > maxContentPerItem * 0.7 ? smartEnd + 1 : maxContentPerItem;
        const truncatedSerialized = serializedHierarchy.substring(0, finalEnd) + '...}';
        
        try {
          // Try to parse back if possible, fallback to string
          truncatedFields.structure = item.structure ? JSON.parse(truncatedSerialized) : undefined;
          truncatedFields.hierarchy = item.hierarchy ? JSON.parse(truncatedSerialized) : undefined;
        } catch {
          // Fallback: store as truncated string
          truncatedFields.structure = item.structure ? truncatedSerialized : undefined;
          truncatedFields.hierarchy = item.hierarchy ? truncatedSerialized : undefined;
        }
    ...
    if (totalChars > targetCharLimit * 0.9) { // 90% threshold for intermediate
      console.log(
        `🎯 [IntermediateTruncation] Reached 90% of limit at item ${index + 1}, stopping here`
      );
      return null;
    }
  ...
  console.log(
    `🎯 [IntermediateTruncation] Reduced from ${data.length} to ${truncatedData.length} items, estimated ${totalChars} chars`
  );
