 ...
 */
export const testReactSearchBasics = async ({
  model,
  target,
  rootUid,
  targetUid,
  ...
  try {
    ...
    if (!parentUid) {
      throw new Error(
        "No valid parent UID found - please focus a block or page first"
      );
    }
    ...
    const responseUid = await createChildBlock(
      parentUid,
      assistantRole + "TEST: ReAct Search Basics"
    );
    ...
    results += `**Query Performance:** ${(endTime - startTime).toFixed(
      2
    )}ms\n\n`;
 ...
 */
export const testFindPagesByTitleLogic = async ({
  model,
  target,
  rootUid,
  targetUid,
  ...
  try {
    ...
    const responseUid = await createChildBlock(
      parentUid,
      assistantRole + "TEST: FindPagesByTitle Logic"
    );
    ...
    testResults.singleCondition = {
      ...
      exact: allPages.filter(([uid, title]) => title === singleCondition)
        .length,
    ...
    const negationResults = allPages.filter(([uid, title]) => {
      const hasFirst = title
        .toLowerCase()
        .includes(searchTerms[0].toLowerCase());
      const hasSecond = searchTerms[1]
        ? title.toLowerCase().includes(searchTerms[1].toLowerCase())
        : false;
    ...
    const totalRegular = allPages.filter(
      ([uid]) => !dnpPattern.test(uid)
    ).length;
    ...
    if (testResults.multipleAND) {
      results += `**Multi-Condition AND Test (${testResults.multipleAND.terms.join(
        " AND "
      )}):**\n`;
    ...
    if (testResults.multipleOR) {
      results += `**Multi-Condition OR Test (${testResults.multipleOR.terms.join(
        " OR "
      )}):**\n`;
 ...
 */
export const testFindPagesSemanticallyLogic = async ({
  model,
  target,
  rootUid,
  targetUid,
  ...
  try {
    ...
    const responseUid = await createChildBlock(
      parentUid,
      assistantRole + "TEST: FindPagesSemantically Logic"
    );
    ...
    const expansionTerms = mockExpansions[searchQuery.toLowerCase()] || [
      "related",
      "similar",
      "connected",
    ];
    ...
    for (const term of expansionTerms) {
      const termResults = allPages.filter(
        ([uid, title]) =>
          title.toLowerCase().includes(term.toLowerCase()) &&
    ...
    // Test DNP filtering
    const dnpPattern = /^\d{2}-\d{2}-\d{4}$/;
    const nonDNPResults = expandedResults.filter(
      ([uid]) => !dnpPattern.test(uid)
    );
    const onlyDNPResults = expandedResults.filter(([uid]) =>
      dnpPattern.test(uid)
    );
    ...
    if (expandedResults.length > 0) {
      ...
      expandedResults
        .slice(0, 5)
        .forEach(([uid, title, created, modified], i) => {
          const isDNP = dnpPattern.test(uid);
          const matchType = exactResults.some((e) => e[0] === uid)
            ? "Direct"
            : "Expanded";
          results += `${i + 1}. [[${title}]] \`${uid}\` (${matchType}, ${
            isDNP ? "DNP" : "Regular"
          })\n`;
        });
    }
 ...
 */
export const testFindBlocksByContentLogic = async ({
  model,
  target,
  rootUid,
  targetUid,
  ...
  try {
    ...
    const responseUid = await createChildBlock(
      parentUid,
      assistantRole + "TEST: FindBlocksByContent Logic"
    );
    ...
    for (const term of searchTerms) {
      ...
      results[`text_${term}`] = {
        ...
        samples: containsMatch
          .slice(0, 2)
          .map(([uid, content, time, pageTitle]) => ({
            uid,
            content:
    ...
    // Test DNP detection
    const dnpPattern = /^\d{2}-\d{2}-\d{4}$/;
    const blocksInDNPs = allBlocks.filter(
      ([uid, content, time, pageTitle, pageUid]) => dnpPattern.test(pageUid)
    );
    const blocksInRegularPages = allBlocks.filter(
      ([uid, content, time, pageTitle, pageUid]) => !dnpPattern.test(pageUid)
    );
    ...
    if (results.multi_condition_AND) {
      resultsText += `**Multi-Condition AND Test (${results.multi_condition_AND.terms.join(
        " AND "
      )}):**\n`;
 ...
 */
export const testReactSearchPerformance = async ({
  model,
  target,
  rootUid,
  targetUid,
  ...
  try {
    ...
    const responseUid = await createChildBlock(
      parentUid,
      assistantRole + "TEST: ReAct Search Performance"
    );
    ...
    for (const test of tests) {
      ...
      results += `- Performance: ${((result.length / duration) * 1000).toFixed(
        0
      )} items/second\n\n`;
    }
    ...
    const filtered = allPages.filter(
      ([uid, title]) => title.toLowerCase().includes("a") // Very common letter
    );
    ...
    results += `- Performance: ${(
      (allPages.length / (filterEnd - filterStart)) *
      1000
    ).toFixed(0)} items/second\n\n`;
 ...
 */
export const testReactAllTools = async ({
  model,
  target,
  rootUid,
  targetUid,
  ...
  try {
    ...
    const responseUid = await createChildBlock(
      parentUid,
      assistantRole + "TEST: ReAct All Tools"
    );
    ...
    try {
      ...
      const samplePages = window.roamAlphaAPI
        .q(
          `[:find ?uid :where [?page :node/title ?title] [?page :block/uid ?uid]]`
        )
        .slice(0, 10);
    ...
    try {
      ...
      const sampleBlocks = window.roamAlphaAPI
        .q(
          `[:find ?uid :where [?b :block/uid ?uid] [?b :block/string ?content] [(> (count ?content) 10)]]`
        )
        .slice(0, 2);
    ...
    results += `**Success Rate:** ${successCount}/${toolCount} (${Math.round(
      (successCount / toolCount) * 100
    )}%)\n`;
    results += `**Total Duration:** ${totalDuration.toFixed(2)}ms\n`;
    results += `**Average per Tool:** ${(totalDuration / toolCount).toFixed(
      2
    )}ms\n\n`;
    ...
    console.log(
      "✅ TEST: ReAct All Tools comprehensive test completed successfully"
    );
  } catch (error) {
