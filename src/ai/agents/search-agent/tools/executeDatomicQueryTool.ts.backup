...
const injectUidFiltering = (
  ...
  if (!whereMatch) {
    console.warn(
      "Could not find :where clause in query, skipping UID injection"
    );
    return query;
  }
  ...
  if (additionalClauses) {
    ...
    return (
      query.slice(0, insertPoint) + additionalClauses + query.slice(insertPoint)
    );
  }
...
const generateDatomicQueryImpl = async (
  input: z.infer<typeof schema>,
  state?: any
): Promise<DatomicQueryResult> => {
  const {
    queryDescription,
    targetEntity,
    searchTerms,
  ...
  console.log(
    `🔍 ExecuteDatomicQuery: ${
      userQuery
        ? "user-provided query"
        : targetEntity + ' search for "' + queryDescription + '"'
    }`
  );
  ...
  if (userQuery) {
    ...
    result.explanation = `User-provided Datalog query${
      variables ? " with " + Object.keys(variables).length + " variables" : ""
    }`;
    result.estimatedComplexity =
      userQuery.length > 200
        ? "high"
        : userQuery.includes("(or ") || userQuery.includes("(and ")
        ? "medium"
        : "low";
    ...
    if (finalBlockUids.length > 0 || finalPageUids.length > 0) {
      result.query = injectUidFiltering(
        result.query,
        finalBlockUids,
        finalPageUids
      );
    ...
    if (executeQuery) {
      try {
        ...
        console.log(
          ...
          } results in ${result.executionTime.toFixed(1)}ms`
        );
      } catch (error) {
  ...
  if (estimateOnly) {
    result.estimatedComplexity =
      searchTerms?.length > 3
        ...
        ? "medium"
        : "low";
    result.explanation = `Estimated ${
      result.estimatedComplexity
    } complexity query for ${targetEntity}s matching: ${searchTerms?.join(
      ` ${conditionLogic} `
    )}`;
    return result;
  }
  ...
  if (!targetEntity || !searchTerms) {
    throw new Error(
      "Either provide a 'query' for direct execution, or 'targetEntity' and 'searchTerms' for auto-generation"
    );
  }
  ...
  // Analyze complexity
  result.estimatedComplexity =
    searchTerms.length > 3 ? "high" : searchTerms.length > 1 ? "medium" : "low";
  ...
  if (!limitResults || limitResults > 500) {
    result.warnings.push(
      "Large result sets may be slow - consider adding a limit"
    );
  }

  // Generate explanation
  result.explanation = `This query searches for ${targetEntity}s containing: ${searchTerms.join(
    ` ${conditionLogic} `
  )}`;
  if (limitToPages && limitToPages.length > 0) {
...
const extractFindVariables = (query: string): string[] => {
  try {
    ...
    if (!findMatch) {
      console.warn(
        "Could not parse :find clause from query:",
        query.slice(0, 100)
      );
    ...
    const cleanVariables = variableMatches.map((variable) =>
    ...
    if (this.combineLogic === "AND" || this.searchTerms.length === 1) {
      ...
      for (const { term, index } of this.searchTerms) {
        ...
        this.whereClause.push(
          `[(re-pattern "(?i)${escapedTerm}") ${regexVar}]`
        );
      ...
      const orClauses = this.searchTerms.map(
        ({ index }) => `[(re-find ?regex${index} ${target})]`
      );
    ...
    if (this.targetEntity === "page") {
      ...
      if (matchType === "exact") {
        if (pageNames.length === 1) {
          ...
          const orClauses = pageNames.map(
            (name) => `[?entity :node/title "${name}"]`
          );
          this.whereClause.push(`(or ${orClauses.join(" ")})`);
        }
      } else if (matchType === "contains") {
        const patterns = pageNames
          .map((name, i) => {
      ...
      if (matchType === "exact") {
        if (pageNames.length === 1) {
          ...
          this.whereClause.push("[?entity :block/page ?target-page]");
          const orClauses = pageNames.map(
            (name) => `[?target-page :node/title "${name}"]`
          );
          this.whereClause.push(`(or ${orClauses.join(" ")})`);
        }
      } else if (matchType === "contains") {
...
export const executeDatomicQueryTool = tool(
  async (input, config) => {
    ...
    try {
      ...
      if (
        queryResult.executionResults &&
        Array.isArray(queryResult.executionResults)
      ) {
        ...
        const formattedResults = queryResult.executionResults.map(
          (resultRow, index) => {
            ...
            findVariables.forEach((variable) => {
              ...
              if (
                propertyMappings.blockUid.some((pattern) =>
                  varLower.includes(pattern)
                )
              ) {
                detectedData.blockUids.push({ variable, value });
              } else if (
                propertyMappings.pageUid.some((pattern) =>
                  varLower.includes(pattern)
                )
              ) {
                detectedData.pageUids.push({ variable, value });
              } else if (
                propertyMappings.content.some((pattern) =>
                  varLower.includes(pattern)
                )
              ) {
                ...
                propertyMappings.title.some((pattern) =>
                  varLower.includes(pattern)
                )
              ) {
            ...
            if (detectedData.blockUids.length > 0) {
              ...
              if (detectedData.contentFields.length > 0) {
                ...
                console.log(
                  `🔍 Inferred page type from title presence (no content)`
                );
              } else {
                ...
                console.log(
                  `⚠️ Fallback: assuming block type for variable '${findVariables[0]}'`
                );
              }
            }

            console.log(
              `🎯 ExecuteDatomicQuery result classified as: ${detectedType} (uid: ${primaryUid})`
            );
