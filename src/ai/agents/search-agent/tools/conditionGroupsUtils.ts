...
export const baseConditionSchema = z.object({
  type: z.enum(["text", "page_ref", "block_ref", "regex", "page_ref_or"]).default("text"),
...
export const conditionGroupSchema = z.object({
  conditions: z.array(baseConditionSchema).min(1, "At least one condition required in group"),
  combination: z.enum(["AND", "OR"]).default("AND").describe("How to combine conditions within this group"),
});
...
export const extendedConditionsSchema = z.object({
  ...
  conditions: z.array(baseConditionSchema).optional().describe("Simple list of conditions for basic logic"),
  combineConditions: z.enum(["AND", "OR"]).default("AND").describe("How to combine simple conditions"),
  
  // Grouped conditions (new advanced feature)  
  conditionGroups: z.array(conditionGroupSchema).optional().describe("Groups of conditions for complex logic like ((A|B) AND NOT C)"),
  groupCombination: z.enum(["AND", "OR"]).default("AND").describe("How to combine condition groups"),
});
...
export const applyORToRegexConversion = (
  ...
  if (combination === "OR" && hasNegatedConditions) {
    ...
    if (positiveConditions.length > 1) {
      ...
      console.log(`🔄 Converted ${positiveConditions.length} positive conditions to regex for OR logic, keeping ${negativeConditions.length} negative conditions`);
...
export const validateConditionInput = (input: any): void => {
  ...
  if (hasGrouped && hasSimple) {
    throw new Error("Cannot use both 'conditions' (simple) and 'conditionGroups' (grouped) simultaneously. Use either simple conditions for basic logic or grouped conditions for complex logic.");
  }
  
  if (!hasGrouped && !hasSimple) {
    throw new Error("Must provide either 'conditions' (simple) or 'conditionGroups' (grouped) for search.");
  }
};
