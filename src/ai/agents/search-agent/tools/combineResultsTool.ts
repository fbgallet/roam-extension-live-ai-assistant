...
import { z } from "zod";
import { createToolResult } from './searchUtils';
...
const resultSetSchema = z.object({
  ...
  type: z.enum(["pages", "blocks"]).describe("Type of entities in this result set"),
  metadata: z.record(z.any()).optional().describe("Additional metadata about the result set")
});

const schema = z.object({
  resultSets: z.array(resultSetSchema).min(2, "At least two result sets are required for combination"),
  operation: z.enum(["union", "intersection", "difference", "symmetric_difference"]).default("union").describe("Set operation to perform"),
  
  // Deduplication options
  deduplicateWithin: z.boolean().default(true).describe("Remove duplicates within each result set before combining"),
  deduplicateAcross: z.boolean().default(true).describe("Remove duplicates across result sets after combining"),
  
  // Merging options
  preserveOrder: z.boolean().default(false).describe("Attempt to preserve original ordering where possible"),
  orderBy: z.enum(["first_appearance", "alphabetical", "frequency", "reverse_frequency"]).default("first_appearance").describe("How to order the final results"),
  
  // Filtering options
  minAppearances: z.number().min(1).default(1).describe("Minimum times a UID must appear across all sets to be included"),
  maxAppearances: z.number().optional().describe("Maximum times a UID can appear across all sets"),
  
  // Output options
  includeStats: z.boolean().default(true).describe("Include statistics about the combination operation"),
  includeSourceInfo: z.boolean().default(false).describe("Include information about which sets each UID came from"),
...
const combineResultsImpl = async (input: z.infer<typeof schema>): Promise<CombinedResult> => {
  const {
    resultSets,
    operation,
    deduplicateWithin,
  ...
  console.log(`🔄 CombineResults: ${operation} operation on ${resultSets.length} result sets`);

  // Validate that all result sets are the same type
  const types = [...new Set(resultSets.map(set => set.type))];
  if (types.length > 1) {
    throw new Error(`Cannot combine different types: ${types.join(', ')}. All result sets must be the same type.`);
  }
  ...
  combinedUids = combinedUids.filter(uid => {
    ...
    return frequency >= minAppearances && 
  ...
  combinedUids = applyOrdering(combinedUids, orderBy, uidFrequency, processedSets, preserveOrder);
...
const performIntersection = (resultSets: any[]): string[] => {
  ...
  if (resultSets.length === 1) return [...new Set(resultSets[0].uids as string[])];

  let intersection = new Set(resultSets[0].uids as string[]);
  
  for (let i = 1; i < resultSets.length; i++) {
    const currentSet = new Set(resultSets[i].uids as string[]);
    intersection = new Set([...intersection].filter(uid => currentSet.has(uid)));
  }
  ...
  switch (orderBy) {
    ...
    default:
      if (preserveOrder) {
        ...
        return [...uids].sort((a, b) => (orderMap[a] || 0) - (orderMap[b] || 0));
      }
 ...
 */
const calculateStats = (processedSets: any[], combinedUids: string[], operation: string) => {
  const totalInputUids = processedSets.reduce((sum, set) => sum + set.uids.length, 0);
...
export const combineResultsTool = tool(
  async (llmInput) => {
    ...
    try {
      ...
      return createToolResult(false, undefined, error.message, "combineResults", startTime);
    }
  },
  {
    name: "combineResults",
