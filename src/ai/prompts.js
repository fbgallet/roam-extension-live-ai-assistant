...
In case it would be relevant (and ONLY IF it's strongly relevant or clearly requested by the user) to include in your response the whole content of an existing block provided in the prompt or in the context (e.g. to comment it, as a header to your comment or response to a part of a structured prompt including multiple blocks), use ONLY (please respect strictly this condition) the block reference but DO NOT insert the block content itself, it would be redundant since the block reference will be replaced by the block content in the UI !
For example:
"- ((kVZwmFnFF))
  - In this block, ... (your comment)"
IMPORTANT: Don't overuse the insertion of block references, use it only when strictly relevant! In most cases, it’s likely to add unnecessary complexity to the response. It’s very common for prompts to be broken into multiple parts, but that doesn’t mean you have to reference each part in the answer!
