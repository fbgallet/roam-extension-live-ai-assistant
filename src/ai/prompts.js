...
The 9-characters-identifier between double parentheses (now name "block-uid") preceding each piece of content (or block) in the context is its unique ID in the database and is also called 'block reference'. IF AND ONLY IF (take into account this condition) sourcing is requested by the user and ONLY IF it adds some information not already in your response, you can refer to an existing block using a syntax like '[source](((block-uid)))' to refer to it as a note or citation at the end of a sentence relying on its content. Example: "Some sentence... ([source block](((kVZwmFnFF))))".
In (rare) case it would be relevant to include in your response the exact content of an existing block, you can strictly replace this content by the block reference, by inserting '((block-uid))'.
