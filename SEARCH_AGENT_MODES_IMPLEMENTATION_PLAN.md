...
### Week 1: Balanced Mode Foundation ✅ CURRENT FOCUS
1. ✅ Document complete plan
2. 🔄 Implement progressive content limits in searchUtils.ts
3. 🔄 Add context expansion detection logic
4. 🔄 Create contextExpansion node with Datomic-based hierarchy

### Week 2: Balanced Mode Refinement
1. Integrate context expansion into routing
2. Update directFormat to handle hierarchical context  
3. Add user transparency messaging
4. Test balanced mode enhancements

### Week 3: Full Mode Enhancement
1. Rebalance limits and add progressive strategy
2. Implement exploration detection logic
3. Create smartPreSelection node
4. Update system prompts for full mode

### Week 4: Full Mode Completion
1. Add content enrichment capabilities
2. Integrate exploration strategies
3. Test and refine both modes
4. Documentation and user guide updates

## Key Files to Modify

### Core Files
- `src/ai/agents/search-agent/tools/searchUtils.ts` - Progressive limits and context expansion logic
- `src/ai/agents/search-agent/ask-your-graph-agent.ts` - New nodes and routing logic
- `src/ai/agents/search-agent/ask-your-graph-prompts.ts` - Result data processing and system prompts

### Supporting Files
- Graph builder updates for new nodes
- Routing logic enhancements
- User interface transparency improvements

## Expected Benefits

1. **Balanced Mode**: Intelligent context when needed, respects Datomic-first philosophy
2. **Full Mode**: Truly differentiated with smart content analysis and exploration
3. **Progressive Limits**: Right level of detail for each result set size
4. **Backward Compatible**: Builds on existing infrastructure without breaking changes
5. **User Transparency**: Clear communication about what's happening and why

## Technical Implementation Notes

### Context Expansion in Balanced Mode
- Uses existing Datomic query infrastructure
- No access to extractHierarchyContent tool
- Implements lightweight hierarchy expansion with custom queries
- Maintains secure-level tool restriction

### Exploration in Full Mode  
- Leverages existing getNodeDetails and extractHierarchyContent tools
- Implements progressive content analysis strategies
- Uses existing result lifecycle management (intermediate, final, completion)
- Maintains content-aware search philosophy

### Progressive Content Strategy
- Automatic adaptation based on result count
- Transparent to user with explanatory messaging
- Optimizes token usage while preserving user experience
- Scales from detailed analysis (few results) to overview (many results)

---

*This plan transforms the modes from similar experiences with different limits into genuinely different search strategies optimized for their respective use cases.*